...
public interface HospitalRepository extends JpaRepository<Hospital, UUID> {
     ...
     */
    @Query(value = "SELECT h.* FROM hospitals h WHERE ST_DWithin(h.location, :origin, :radiusMeters) ORDER BY ST_Distance(h.location, :origin) ASC", 
           countQuery = "SELECT count(*) FROM hospitals h WHERE ST_DWithin(h.location, :origin, :radiusMeters)",
           nativeQuery = true)
    Page<Hospital> findWithinRadius(
    ...
    @Query(value = "SELECT DISTINCT h.* FROM hospitals h " +
            ...
            countQuery = "SELECT count(DISTINCT h.id) FROM hospitals h " +
            "JOIN blood_inventory bi ON bi.hospital_id = h.id " +
            "WHERE ST_DWithin(h.location, :origin, :radiusMeters) " +
            "  AND h.verified = true " +
            "  AND bi.blood_type = :bloodType " +
            ...
            "  AND bi.units_available > 0",
            nativeQuery = true)
    Page<Hospital> findNearbyWithAvailableBlood(
            @Param("origin") Point origin,
            @Param("radiusMeters") double radiusMeters,
